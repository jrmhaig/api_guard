require 'api_guard/access_token/cookie'
require 'action_dispatch'
require 'active_support/testing/time_helpers'

include ActiveSupport::Testing::TimeHelpers

RSpec.describe ApiGuard::AccessToken::Cookie do
  ...
  describe '#store' do
    ...
    it { expect { store }.to change { packet.cookies['access_token'] }.to(value) }

    context 'with an expire at time set' do
      let(:validity) { 37 * 60 }
      let(:expire_time) { current_time + validity }

      it { expect { store }.to change { packet.headers['Set-Cookie']}.to match(/access_token.*expires=#{expire_time.strftime('%a, %d %b %Y')}.*HttpOnly/) }
    end
  ...
  describe '#fetch' do
    ...
    context 'with a value' do
      ...
      before { packet.cookies['access_token'] = value }
